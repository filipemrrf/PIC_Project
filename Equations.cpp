...
void Wave_Equation_4th_Order(double* u, int N, double step_x, double* params){
    ...
    for(int i = 0; i < N/2; ++i){
        ...
        if((i == 0) || (i == (N/2 - 1)))
            Pi1[i] = k*(-Phi0[2] + 16.0*Phi0[1] - 30.0*Phi0[0] + 16*Phi0[N/2-2] - Phi0[N/2-3]);
        if(i == 1)
            Pi1[1] = k*(-Phi0[3] + 16.0*Phi0[2] - 30.0*Phi0[1] + 16*Phi0[0] - Phi0[N/2-2]);
        if(i == (N/2 - 2))
            Pi1[N/2-2] = k*(-Phi0[1] + 16.0*Phi0[0] - 30.0*Phi0[N/2-2] + 16*Phi0[N/2-3] - Phi0[N/2-4]);
        else
            Pi1[i] = k*(-Phi0[i+2] + 16.0*Phi0[i+1] - 30.0*Phi0[i] + 16*Phi0[i-1] - Phi0[i-2]);
    }
